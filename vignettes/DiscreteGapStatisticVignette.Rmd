...
- Perform simulations that (a) reproduce one of the scenarios found in the publication and (b) explore the cluster-selection proportion given a dataset. These exercises are intended to help extend this tool's usage.
...
The MathAnxiety data is loaded from the `likert` package. After doing some light formatting a `data.frame` object is obtained called `massData`. The possible categories are 'Strongly Disagree', 'Disagree', 'Neutral' , 'Agree', 'Strongly Agree'. Notice that the categories are not transformed to numerical values, but are abbreviated downstream for plotting purposes to DS, D, N, A, SA. Category ordering is preserved using the factor function. 
...
The first function is an adaptation of the `likert.heatmap` found in the `likert` R package. `likert.heat.plot2` has the same arguments as the original function, but has an additional parameter, `allLevels`, where category order is specified in a vector. Another difference in this function is the output. Since the original function numerically codifies categories, mean and variance statistics are produced at the left-hand side of the plot. These statistics are not present in plots produced by `likert.heat.plot2`.
...
In ___ five categorical distance functions are introduced to quantify dissimilarities/discrepancies between two categorical vectors (see function `distancematrix`). The following table describes the available distances and the names used within the package.
...
The resulting distance matrix from rectangular datasets can easily be displayed and organized using heatmaps. The following plots visualize the Math Anxiety dataset using the defined distances using the function `distanceHeat`. This function only requires the dataset object and the name of the distance. It can also take advantage of the options and functionalities available in the `pheatmap` function (see the code below). For instance, by default, the columns are clustered using `clustering_method = 'complete'` but this parameter can be specified according to `pheatmap`'s options. Different options are explored in the plots below using the different functions introduced. The clusters obtained here might not necessarily reflect the ones obtained using `clusGapDiscr`. 
...
The Gap Statistic (GS) in its original formulation aims to determine an optimal number of clusters given a rectangular dataset with continuous columns and a pre-specified clustering method given $k$ number of clusters (i.e. $k$-means clustering, partitioning around medoids (pam)). This is done by comparing the within-cluster dispersion of the data to a randomly generated reference null distribution of the data obtaining estimations via bootstrapping. The idea is to quantify whether the clustering structure observed in the data is considerably different compared to what would be expected by random chance. `clusGap` is a widely used implementation of the GS from the `cluster` package with the following basic arguments:
...
The discrete GS (dGS) proposed applies and adapts the principle behind the original formulation, but for categorical data using a distance-based approach. The GS assumes that the distance between observations can be correctly described with the Euclidean distance, since assumes the data is continuous. In the case of categorical data, this distance assumption is not appropriate and a different kind of distances need to be defined. `clusGapDiscr` is the main function that performs and implements the dGS. The first four parameters found in `clusGap` are identical and have the same usage to the ones found in `clusGapDiscr`. Additionally, the function requires a categorical distance from the list mentioned above. Another feature of the dGS is the reference null distribution used, which can be a discrete uniform distribution of the unique column-wise categories found in the data; this setting is defined as the Data Support (DS). In other settings, the categories to be used must be user-specified via a character vector or list. This setting will be referred as the Known Support (KS). 
...
This example reproduces results from the simulation study found in ___ .
...
The following code first performs the Monte Carlo simulations, retrieves results and plots them using stacked bars. The simulation piece first generates data using the native `SimData` function requiring `N`  number of observations, `nQ` number of questions and a probability vector. Then `clusGapDiscr` function is called using the parameters above. The result would a be `list` with the chosen number of clusters for each replication. A `data.frame` formats the results to be displayed via `ggplot2`.
...
ggplot(resSim, aes(x=dist, y=Freq, fill = nClust)) +
   ...
   theme_bw() +
   labs(title = 'Simulation Results (100 repetitions)\n', 
...
The stacked-bar plot summarizes the results of the simulations. The red color highlights correct number of cluster choices, while different tones of grey indicate the wrong choices. Here the two distances perform similarly, except the case where $\chi^2$ distance does not underestimate number of clusters compared to the Bhattacharyya distance, choosing 1-group cluster $7%$ of the time.


## Simulating a real dataset

Notice that the likert heatmap from the Math Anxiety data displayed above contains fairly different question-wise probability profiles. For instance, for Q3 only three categories have non-zero probabilities. Consolidating these question profiles can help to model and simulate datasets with similar profiles and examine then number of clusters chosen running a Monte Carlo simulation. For this particular task the `likert` function from the `likert` package helps to summarize the question probability profiles.   

```{r Sim2Run, echo=FALSE, eval=FALSE, fig.align='center', out.width=400, dpi=500}
