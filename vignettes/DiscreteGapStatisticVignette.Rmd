...
- Perform simulations that (a) reproduce one of the scenarios found in the publication and (b) explore the cluster-selection proportion given a dataset. These can help to extend usage of the present tool.
...
The MathAnxiety data is loaded from the `likert` package. After doing some light formatting a `data.frame` object is obtained called `massData`. The items are shortened into meaningful words that identify them. Notice that the categories are not transformed to numerical values, but are abbreviated for downstream plotting purposes. Category ordering is preserved using the factor function. 
...
# Summarizationation and Data Exploration
...
$5$ distance functions are introduced to measure dissimilarities between two categorical vectors (see function `distancematrix`). The following table describes the available distances and the names used within the package.
...
distanceHeat(x = massData[, -1], distName = 'hellinger',
             ...
             clustering_method = 'single', show_rownames = FALSE, border_color = 'black')

distanceHeat(x = massData[, -1], distName = 'bhattacharyya',
             main = 'Bhattacharyya Distance\nMath Anxiety Data', fontsize = 6.5, 
             clustering_method = 'centroid', show_rownames = FALSE, border_color = 'lightgrey')
```
...
```{r resHeatMapsPlot, echo=FALSE, eval=TRUE, out.width=215, dpi=500, message=FALSE, warning=FALSE}
...
- Replicate a scenario from the paper
- Running a simulation based on the profiles extracted from the data.
